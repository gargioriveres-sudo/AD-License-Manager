...
# ─── Entrada — remove \r e espaços laterais ───────────────────────────────────
_trim() {
  local v="$1"
  v="${v//$'\r'/}"
  v="${v#"${v%%[![:space:]]*}"}"
...
step_0_verify() {
  ...
  if   [ "$RAM_GB" -lt "$REQUIRED_RAM_GB" ]; then
    ...
    warn "RAM: ${RAM_GB}GB — recomendado 8GB+."
  else
    ok "RAM: ${RAM_GB}GB"
  fi
  ...
  if   [ "$DISK_GB" -lt "$REQUIRED_DISK_GB" ]; then
    fail "Disco insuficiente: ${DISK_GB}GB. Mínimo: ${REQUIRED_DISK_GB}GB."
  elif [ "$DISK_GB" -lt 40 ]; then
    warn "Disco: ${DISK_GB}GB livres — recomendado 40GB+."
  else
  ...
  ok "Verificações iniciais passaram"
  pause
}
...
step_1_collect() {
  ...
  echo -e "  ${DIM}Padrões entre [ ] — ENTER para aceitar.${NC}"
  echo ""
  ...
  echo ""
  info "Exemplos: admanager.empresa.com.br · portal.ti.empresa.org.br"
  echo ""
  while true; do
  ...
  echo ""
  info "Use ldaps:// (636) para reset de senha."
  info "Use ldap://  (389) somente se LDAPS não estiver disponível."
  echo ""
  while true; do
    ...
    local _port
    _port=$(echo "$AD_URL" | grep -oP ':\K[0-9]+' || true)
    if [ -z "$_port" ]; then
      [[ "$AD_URL" == ldaps://* ]] && AD_PORT="636" || AD_PORT="389"
    else
      AD_PORT="$_port"
    fi
  ...
  while true; do
    ...
    ok "Base DN: ${AD_BASE_DN}"
    break
  done

  echo ""
  info "A conta de serviço precisa de permissões de leitura/escrita no AD."
  echo ""
  while true; do
  ...
  echo ""
  div
  echo -e "  ${WHITE}${BOLD}1.3  Administrador inicial${NC}"
  echo ""
  while true; do
  ...
  echo ""
  info "Senha: mín. 12 chars, maiúsculas, minúsculas, números e símbolos."
  echo ""
  while true; do
    ...
    local CONF
    CONF=$(ask_secret "Confirme a senha")
    if [ "$ADMIN_PASSWORD" != "$CONF" ]; then
  ...
  echo ""
  div
  echo -e "  ${WHITE}${BOLD}1.4  Azure AD / Microsoft 365 ${DIM}(opcional)${NC}"
  echo ""
  info "Necessário para licenças M365, MFA e sessões."
  echo ""
  if confirm "Configurar Azure AD / M365 agora" "s"; then
    ...
    while true; do
      AZURE_CLIENT_SECRET=$(ask_secret "Client Secret")
  ...
  if confirm "Configurar SMTP agora" "s"; then
    SETUP_SMTP="y"
    while true; do
      SMTP_HOST=$(ask "Servidor SMTP" "smtp.empresa.com.br")
    ...
    SMTP_USER=$(ask "Usuário SMTP (em branco para relay)")
    SMTP_USER="${SMTP_USER//$'\r'/}"
    [ -n "$SMTP_USER" ] && SMTP_PASS=$(ask_secret "Senha SMTP")
    while true; do
      SMTP_FROM=$(ask "Email remetente" "admanager@${APP_DOMAIN}")
      SMTP_FROM="${SMTP_FROM//$'\r'/}"
      is_valid_email "$SMTP_FROM" && break
      warn "Formato inválido."
    done
    ok "SMTP: ${SMTP_HOST}:${SMTP_PORT}"
  else
    SMTP_FROM="admanager@${APP_DOMAIN}"
    warn "SMTP ignorado. Configure depois em Configurações."
  fi
  ...
  echo ""
  info "Crie um Incoming Webhook no Teams para obter a URL."
  echo ""
  if confirm "Configurar alertas no Teams agora" "n"; then
    SETUP_TEAMS="y"
    while true; do
      TEAMS_WEBHOOK_URL=$(ask "URL do Incoming Webhook")
      TEAMS_WEBHOOK_URL="${TEAMS_WEBHOOK_URL//$'\r'/}"
  ...
  echo -e "    URL: https://${APP_DOMAIN}  |  IP: ${SERVER_IP}"
  echo ""
  echo -e "  ${CYAN}Active Directory${NC}"
  echo -e "    URL:    ${AD_URL}"
  echo -e "    BaseDN: ${AD_BASE_DN}"
  echo -e "    Conta:  ${AD_USERNAME}"
  echo ""
  echo -e "  ${CYAN}Microsoft 365${NC}"
  if [ "$SETUP_GRAPH" = "y" ]; then
    echo -e "    ${GREEN}Habilitado — Tenant: ${AZURE_TENANT_ID}${NC}"
  else
    echo -e "    ${DIM}Não configurado${NC}"
  fi
  echo ""
  echo -e "  ${CYAN}Administrador${NC}"
  echo -e "    ${ADMIN_USER}  |  ${ADMIN_EMAIL}"
  echo ""
  echo -e "  ${CYAN}SMTP${NC}"
  if [ "$SETUP_SMTP" = "y" ]; then
    echo -e "    ${GREEN}${SMTP_HOST}:${SMTP_PORT}${NC}"
  else
    echo -e "    ${DIM}Não configurado${NC}"
  fi
  echo ""
  ...
  case "$CERT_OPCAO" in
    1) echo -e "    ${GREEN}Let's Encrypt${NC}" ;;
    2) echo -e "    ${YELLOW}PKI corporativa (self-signed temporário)${NC}" ;;
    3) echo -e "    ${DIM}Self-signed 10 anos${NC}" ;;
  esac
  echo ""
  div
  echo ""
...
step_2_prepare() {
  ...
  q apt-get install -y -qq \
    curl wget git openssl netcat-openbsd python3 jq unzip \
...
step_3_security() {
  ...
  q ufw allow https
  q ufw --force enable
  ok "UFW ativo: portas 22, 80 e 443"
...
EOF
  ...
  ok "fail2ban ativo (3 tentativas → bloqueio 2h)"
...
step_4_docker() {
  ...
  sub "Adicionando chave GPG oficial..."
  ...
  sub "Adicionando repositório..."
  echo \
    "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] \
https://download.docker.com/linux/ubuntu ${UBUNTU_CODENAME} stable" \
    > /etc/apt/sources.list.d/docker.list
  q apt-get update -qq
  ok "Repositório adicionado"
...
EOF
  ...
  q docker run --rm hello-world \
    ...
    || fail "Docker com problema. Veja: ${INSTALL_LOG}"
...
# AD License Manager — Gerado em $(date '+%Y-%m-%d %H:%M:%S')
...
echo "\$(date '+%Y-%m-%d %H:%M:%S') Renovado com sucesso." >> ${INSTALL_DIR}/logs/certbot.log
EOF
      ...
      warn "Let's Encrypt falhou. Usando self-signed."
      CERT_OPCAO="3"
    fi
  fi
  ...
  if [ "$CERT_OPCAO" != "1" ] || [ ! -f "${SSL_DIR}/cert.pem" ]; then
    ...
    openssl req -x509 -nodes -days 3650 -newkey rsa:4096 \
      ...
      >> "$INSTALL_LOG" 2>&1 \
      || fail "Falha ao gerar certificado self-signed."
  ...
  ok "Certificado válido — expira: ${EXPIRY}"
}
...
step_8_build() {
  ...
  cd "${INSTALL_DIR}"
  info "Pode levar 10 a 25 minutos."
  info "Acompanhe: tail -f ${INSTALL_LOG}"
  echo ""
  ...
  docker compose build \
    ...
    --progress=plain \
    >> "$INSTALL_LOG" 2>&1 \
    || fail "Erro no build. Detalhes: ${INSTALL_LOG}"
  ok "Todas as imagens construídas"
...
log "=== Backup concluído: $(du -sh "$BACKUP_DIR" | cut -f1) em ${BACKUP_DIR} ==="
BACKUP_SCRIPT
  ok "backup.sh criado"
...
if [ "$FALHAS" -gt 0 ]; then
  ...
  echo "$(date '+%Y-%m-%d %H:%M:%S') FALHA: ${FALHAS}" >> "$LOG"
  exit 1
else
...
RESTORE_SCRIPT
  ...
  ok "Todos os scripts criados com permissões configuradas"
}
...
# =============================================================================
step_12_cron() {
  step "12" "CONFIGURAÇÃO DE CRON JOBS"

  cat > /etc/cron.d/admanager << EOF
# AD License Manager — Tarefas automáticas — $(date '+%Y-%m-%d')
...
EOF
  ...
  ok "  Docker prune diário às 04:00"
}
